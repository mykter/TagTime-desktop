/**
 * This script fixes up the shrinkwrap file to work around a bug.
 *
 ...
 * the `package.json`, and manually remove them from `npm-shrinkwrap.json`
 * if they exist.
 *
 * Courtesy of https://github.com/resin-io/etcher (Apache 2.0)
 *
...
const getDependencyTree = (shrinkwrap, shrinkwrapPath) => {
  ...
  const result = _.map(dependencies, (version, name) => {
    const dependencyPath = _.has(object.dependencies, name)
      ? _.concat(shrinkwrapPath, [name])
      : [name];
...
const blacklist = _.reduce(
  platformSpecificDependencies,
  (accumulator, dependencyPath) => {
